'use strict';
// Espace de noms global du jeu + outils mathématiques, aléatoire et bruit.
const CM = (window.CM = {});
...
// Hachage de coordonnées entières -> [0, 1).
CM.hash3 = function (x, y, z, seed) {
...
// Bruit simplex 2D/3D (d'après Stefan Gustavson), initialisé par une graine.
...
// Matrices 4x4 (colonne-majeure, comme WebGL).
CM.mat4 = {
  create() {
  ...
  fps(out, px, py, pz, yaw, pitch) {
    ...
    out[2] = bx; out[6] = by; out[10] = bz; out[14] = -(bx * px + by * py + bz * pz);
    out[3] = 0; out[7] = 0; out[11] = 0; out[15] = 1;
    return out;
  },
  // Transformation affine simple: translation + rotations Y puis X puis Z.
  compose(out, tx, ty, tz, ry, rx, rz, s) {
...
// Extraction des plans du tronc de vision (frustum) depuis viewProj.
CM.frustumPlanes = function (m, out) {
  const rows = [
...
CM.aabbInFrustum = function (planes, x0, y0, z0, x1, y1, z1) {
  for (let i = 0; i < 6; i++) {
    const p = planes[i];
    const x = p[0] > 0 ? x1 : x0;
    const y = p[1] > 0 ? y1 : y0;
